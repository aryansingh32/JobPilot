...
async function buildApp() {
    ...
    app.post('/execute', { preHandler: authMiddleware }, async (req, reply) => {
        const { siteId, task, sessionId = randomUUID(), userId = 'anonymous', priority = 'normal', useCache = true, dryRun = false, } = req.body;
        if (!siteId || !task)
    ...
    app.get('/jobs/:jobId/runtime', { preHandler: authMiddleware }, async (req, reply) => {
        ...
        if (!runtime)
            return reply.status(404).send({ error: 'Runtime state not found' });
    ...
    app.post('/test/plan', { preHandler: authMiddleware }, async (req, reply) => {
        ...
        const fallbackUrl = body.pageUrl
            ...
            ?? undefined;
        const decision = await getAIPlanner().planTask(body.task, body.siteId, buildSyntheticSnapshot(fallbackUrl), [], body.useCache ?? true);
        return {
