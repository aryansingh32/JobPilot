...
export async function registerAdminRoutes(app) {
    ...
    app.get('/admin/jobs', { preHandler: adminAuth }, async (req, reply) => {
        const { status, userId, limit = '50', offset = '0', from, to } = req.query;
        const pool = getPgPool();
        const conditions = [];
        const params = [];
        let pi = 1;
        ...
        try {
            const { rows } = await pool.query(`SELECT *, ${JOB_ERROR_SQL} FROM job_logs ${where} ORDER BY started_at DESC LIMIT $${pi} OFFSET $${pi + 1}`, params);
            const countRes = await pool.query(`SELECT COUNT(*) FROM job_logs ${where}`, params.slice(0, -2));
    ...
    app.get('/admin/jobs/:jobId', { preHandler: adminAuth }, async (req, reply) => {
        ...
        try {
            ...
            if (!jobRes.rows.length && !runtimeRes)
    ...
    app.get('/admin/workflows', { preHandler: adminAuth }, async (req, reply) => {
        const { siteId, isActive, limit = '100', offset = '0' } = req.query;
        const pool = getPgPool();
        const conditions = [];
        const params = [];
        let pi = 1;
        if (siteId) {
            conditions.push(`site_id = $${pi++}`);
            params.push(siteId);
        }
        ...
        params.push(parseInt(limit), parseInt(offset));
        try {
            const { rows } = await pool.query(`SELECT * FROM site_workflows ${where} ORDER BY created_at DESC LIMIT $${pi} OFFSET $${pi + 1}`, params);
    ...
    app.post('/admin/workflows', { preHandler: adminAuth }, async (req, reply) => {
        ...
        const starterActionPlan = body.starterActionPlan ?? [];
      ...
      `, [
            ...
            if (admin) {
                await pool.query(`INSERT INTO workflow_audit_log (admin_id, workflow_id, action, diff) VALUES ($1, $2, $3, $4)`, [admin.id, rows[0].id, 'CREATE', JSON.stringify({ new: rows[0] })]);
            }
    ...
    app.put('/admin/workflows/:workflowId', { preHandler: adminAuth }, async (req, reply) => {
        ...
        try {
            ...
            const starterActionPlan = body.starterActionPlan !== undefined ? body.starterActionPlan : oldWorkflow.starter_action_plan;
            ...
            const { rows } = await pool.query(`UPDATE site_workflows SET ${sets.join(', ')}, updated_at = NOW() WHERE id = $${pi} RETURNING *`, params);
            if (!rows.length)
                return reply.status(404).send({ error: 'Workflow not found' });
            const admin = req.admin;
            if (admin && oldRows.rows.length) {
                await pool.query(`INSERT INTO workflow_audit_log (admin_id, workflow_id, action, diff) VALUES ($1, $2, $3, $4)`, [admin.id, workflowId, 'UPDATE', JSON.stringify({ old: oldRows.rows[0], new: rows[0] })]);
            }
    ...
    app.delete('/admin/workflows/:workflowId', { preHandler: adminAuth }, async (req, reply) => {
        ...
        try {
            ...
            const admin = req.admin;
            if (admin) {
                await pool.query(`INSERT INTO workflow_audit_log (admin_id, workflow_id, action, diff) VALUES ($1, $2, $3, $4)`, [admin.id, workflowId, 'DELETE', JSON.stringify({ old: oldRows.rows[0] })]);
            }
    ...
    app.get('/admin/logs', { preHandler: adminAuth }, async (req, reply) => {
        const { service = 'api', limit = '200' } = req.query;
        try {
            const redis = await getRedisClient();
            const key = `logs:${service}`;
            const raw = await redis.lRange(key, -parseInt(limit), -1).catch(() => []);
            const entries = raw.map((r) => { try {
                return JSON.parse(r);
            }
            catch {
                return { msg: r };
    ...
    app.get('/admin/errors', { preHandler: adminAuth }, async (req, reply) => {
        ...
        try {
            const redis = await getRedisClient();
            const raw = await redis.lRange('logs:errors', -parseInt(limit), -1).catch(() => []);
            const entries = raw.map((r) => { try {
                return JSON.parse(r);
            }
            catch {
    ...
    app.get('/admin/sites', { preHandler: adminAuth }, async (req, reply) => {
        ...
        const pool = getPgPool();
        try {
            const { rows } = await pool.query(`SELECT id, domain, page_count, status, created_at, updated_at FROM sites ORDER BY created_at DESC LIMIT $1 OFFSET $2`, [parseInt(limit), parseInt(offset)]);
