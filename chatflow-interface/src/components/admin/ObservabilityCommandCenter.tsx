...
export function ObservabilityCommandCenter() {
  ...
  useEffect(() => {
    ...
    const socket: Socket = io(`${origin}/admin`, {
      ...
      withCredentials: true, // sends the httpOnly jp_admin_session cookie
  ...
  const cards = useMemo(
    () => [
      ...
      {
        label: "Sessions 24h",
        value: summary?.sessions24h ?? "—",
        tone: "from-primary/20 to-fuchsia-500/10",
      },
      {
        label: "Errors 24h",
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Radio
            className={`h-4 w-4 ${connected ? "text-emerald-400 animate-pulse" : "text-zinc-500"}`}
          />
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="flex max-h-[320px] flex-col rounded-2xl border border-border/40 bg-black/30">
          <div className="border-b border-border/30 px-4 py-2 text-xs font-medium text-muted-foreground">
            Live feed
          </div>
