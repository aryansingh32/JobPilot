...
export function SitesPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSites(r.sites); setTotal(r.total);
      ...
      <div className="rounded-2xl border border-border/40 bg-card/30 overflow-hidden">
        ...
        <div className="divide-y divide-border/20">
          {loading && !sites.length ? (
            ...
            <div key={s.id} className="grid grid-cols-[2fr_1fr_1fr_1fr_auto] hover:bg-accent/20 transition-colors">
              <div className="px-4 py-3 flex items-center gap-2">
                <Globe className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
                <span className="text-sm text-foreground font-medium truncate">{s.domain}</span>
              </div>
              <div className="px-4 py-3 text-sm text-muted-foreground tabular-nums">{(s.page_count ?? 0).toLocaleString()}</div>
              <div className="px-4 py-3"><StatusBadge status={s.status ?? "active"} /></div>
